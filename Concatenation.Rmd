...
The 'nested' part here deals with multiple sequence runs (i.e., we ran three lanes first to check our DoC, then calculated we needed to run an additional 6 lanes of our library that had 96 samples pooled). But, if you have a simpler scenario (all in one run), you can just amend to use without the nested part. 
...
####The nest step in the workflow is evaluation of data quality with [FastQC][1]
